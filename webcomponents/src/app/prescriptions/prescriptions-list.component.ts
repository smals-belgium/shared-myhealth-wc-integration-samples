...
@else {
    ...
    <div class="footer">
      ...
      /
      <span>{{this.specs?.language}}</span>
      /
      <span>{{this.specs?.configName}}</span>
      /
...
export class PrescriptionsListComponent extends BaseWebComponent  {
  ...
  override async onInitialized() {
    ...
    this.specs.services.registerRefreshCallback( (done) => this.onRefreshData(done) )
  }
  ...
  async updatePrescriptions(force:boolean): Promise<void> {
      const prescriptions = await this.prescriptionsService.getPrescriptions("LIST", this.specs.services, force)

  ...
  template: `
    <div>
      <div style="position:relative; width:100%;">
        <div [ngStyle]="{'opacity':listOpacity}" style="float:left; width:30%; border-right:1px solid #555;" #listWrapper>
          <div>
        ...
        <div style="float:right; width:69%; border:0px solid blue;" #detailsWrapper>
...
export class SamplePrescriptionsComponent implements OnInit {
  ...
  listWrapper = viewChild.required<ElementRef>('listWrapper')
  detailsWrapper = viewChild.required<ElementRef>('detailsWrapper')

  prescriptionsListComponent: any
  prescriptionsDetailsComponent: any
  ...
  ngOnInit(): void {
    ...
    this.prescriptionsListComponent = this.createComponent("sample-prescriptions-list")
    this.prescriptionsListComponent.addEventListener("onSelectedPrescription", ($event: any) => this.onSelectedPrescription($event))
    this.prescriptionsListComponent.addEventListener("onError", ($event: any) => this.onError($event))

    this.prescriptionsDetailsComponent = this.createComponent("sample-prescriptions-details")
    this.prescriptionsDetailsComponent.version = this.version
    this.prescriptionsDetailsComponent.pid = this.prescriptionId;
    this.prescriptionsDetailsComponent.addEventListener("onError", ($event: any) => this.onError($event))

    this.listWrapper().nativeElement.append(this.prescriptionsListComponent)
    this.detailsWrapper().nativeElement.append(this.prescriptionsDetailsComponent)
  ...
  private createComponent(name: string): any {
    var component: any
    component = document.createElement(name)
    component.version = this.version
    component.language = this.language
    component.configName = this.configName
    component.services = this.componentServices;
    return component
  }
